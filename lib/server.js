...
server.onRequest = function(req, res, reqBody) {
    console.log(reqBody)
    var _this = this;
...
server.onPhantomPageCreate = function(req, res) {
    ...
    req.prerender.page.run(_this.options.blockedResources || blockedResources, !!_this.options.logRequests, function(blockedResources, logRequests, resolve, reject) {
        ...
        this.settings.userAgent = this.settings.userAgent;
    ...
    _this._pluginEvent("onPhantomPageCreate", [_this.phantom, req, res], function() {
        ...
        (function(){
            console.log(req.prerender.body)
            if(req.prerender.body){
                return req.prerender.page.run(req.prerender.body, req.prerender.url, function(body, url, resolve, reject) {
                    this.setContent(body, url, function(status) {
                        resolve(status);
                    });
                })
            }
            else{
                return req.prerender.page.run(encodeURI(req.prerender.url).replace('%2523', '%23'), function(url, resolve, reject) {
